...
export const sendEmailInvite = async (
     ...
     */
    const rawToken = new Uint32Array(3);
    crypto.getRandomValues(rawToken)
    const token = rawToken.join('')

    const resend = new Resend(process.env.RESEND_API_KEY);
    /**
     * The html email template requires:
     * - return link with token, i.e. "www.ourhousetheapp.com/invite/{token}"
     * - details about the person who sent the invite, i.e. The senderName
     */
