  ...
  return (
    <div>
      <p>
        ...
        <strong>{invite.household.name}</strong> by {invite.inviterUser.name}.
      </p>
      <p>
        The invite is for: <strong>{invite.invitedEmail}</strong>.
      </p>
      <p>Please sign in with Google to accept the invite.</p>
      <AuthButton
        size="small"
        completeInviteUrl={`/invite/complete?inviteToken=${token}`}
      />
    </div>
  );
}
